...
class Role extends Model implements JsonSerializable{
	...
	public function set($id,$role_name,$created_at,$updated_at){
		$this->id=$id;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}roles(role_name,created_at,updated_at)values('$this->role_name','$this->created_at','$this->updated_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}roles set role_name='$this->role_name',created_at='$this->created_at',updated_at='$this->updated_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,role_name,created_at,updated_at from {$tx}roles");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,role_name,created_at,updated_at from {$tx}roles $criteria limit $top,$perpage");
		$data=[];
		while($role=$result->fetch_object()){
			$data[]=$role;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,role_name,created_at,updated_at from {$tx}roles where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Role Name</th><th>Created At</th><th>Updated At</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Role Name</th><th>Created At</th><th>Updated At</th></tr>";
		}
		while($role=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$role->id</td><td>$role->role_name</td><td>$role->created_at</td><td>$role->updated_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
	...
	static function html_row_details($id){
		...
		$html.="<tr><th>Role Name</th><td>$role->role_name</td></tr>";
